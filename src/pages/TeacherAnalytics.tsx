    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="font-bold text-gray-600">Deep dive into class performance, mood, and learning trends.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-blue-50 border-blue-400 p-6 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="flex items-center gap-1 text-green-600 font-black text-xs">
              <ArrowUp size={14} /> 5%
            </span>
          </div>
          <p className="text-xs font-black uppercase text-gray-500">Avg. Accuracy</p>
        ...
        <Card className="bg-yellow-50 border-yellow-400 p-6 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-black text-gray-500">Stable</span>
          </div>
          <p className="text-xs font-black uppercase text-gray-500">Dominant Mood</p>
          <p className="text-4xl font-black uppercase">OK</p>
        </Card>
        <Card className="bg-red-50 border-red-400 p-6 space-y-2">
          <div className="flex items-center justify-between">
            <AlertCircle size={24} className="text-red-600" />
            <span className="text-xs font-black text-red-600">Action Needed</span>
          </div>
          <p className="text-xs font-black uppercase text-gray-500">Weak Topics</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="p-8 space-y-6">
          <h2 className="text-2xl font-black flex items-center gap-2">
            <AlertCircle className="text-red-600" /> Struggling Students
          </h2>
          <div className="space-y-4">
            {analytics.strugglingStudents?.map(student => (
              <div key={student.id} className="flex items-center justify-between p-4 neo-border bg-red-50">
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    ...
                    <p className="font-black text-red-600">{student.score}%</p>
                  </div>
        ...
        <Card className="p-8 space-y-6">
          <h2 className="text-2xl font-black flex items-center gap-2">
            <TrendingUp className="text-green-600" /> Top Performers
          </h2>
          <div className="space-y-4">
            {analytics.topPerformers?.map(student => (
              <div key={student.id} className="flex items-center justify-between p-4 neo-border bg-green-50">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 neo-border bg-white flex items-center justify-center font-black">
                    {student.name[0]}
                  </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="text-right">
                    <p className="text-[10px] font-black uppercase text-gray-500">Accuracy</p>
                    <p className="font-black text-green-600">{student.score}%</p>
                  </div>
                  <Link to={`/teacher/students/${student.id}`}>
                    <Button className="p-2">
                      <ChevronRight size={18} />
      ...
      <Card className="p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {analytics.weakTopics?.map(topic => (
            <div key={topic} className="p-6 neo-border bg-gray-50 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="h-3 neo-border bg-white overflow-hidden">
